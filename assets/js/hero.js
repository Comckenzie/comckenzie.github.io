(function(){
  ...
  async function initHeroParallax(){
    ...
    for (let i = 0; i < Math.min(N, cells.length); i++) {
      ...
      img.src = box.meta.src; img.alt = ''; img.loading = 'lazy'; img.decoding = 'async';
      Object.assign(img.style, { width: '100%', height: '100%', objectFit: 'contain', pointerEvents: 'none', animationDuration: (2.5 + Math.random() * 2.5).toFixed(2) + 's', animationDelay: (Math.random() * 4).toFixed(2) + 's' });
    ...
    const applyTransforms = centered => {
      for (const s of squares) {
        ...
        const im = s.querySelector('img'); if (im) im.style.opacity = o;
      }
    };
    ...
    const init = () => {
      ...
      setTimeout(() => { for (const s of squares) s.style.transition = 'transform 0.35s cubic-bezier(0.22,1,0.36,1)'; }, 30);
    };
    ...
    function update() {
      ...
      const ease = 0.02; cur += (target - cur) * ease; applyTransforms(cur);
